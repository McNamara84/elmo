...
describe("buttons.js", () => {
  ...
  function loadScript() {
    let script = fs.readFileSync(path.resolve(__dirname, "../../js/eventhandlers/buttons.js"), "utf8");
  ...
  beforeEach(() => {
    ...
    window.jQuery = $;
    $.fn.modal = jest.fn();
    $.fn.load = jest.fn(function (url, cb) {
      if (cb) cb();
      return this;
    });
    $.fn.tooltip = jest.fn();
  ...
  test("hovering 30 times opens Easter egg", () => {
    ...
    expect(window.open).toHaveBeenCalledWith("doc/egg.html", "Egg", "width=650,height=450,scrollbars=no,resizable=no,location=no");
  });
