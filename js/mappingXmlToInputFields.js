...
// ─── ICGEM-specific parsing ───────────────────────────────────────────────────

const ICGEM_NAMESPACE_URI = 'http://icgem.gfz.de/schema';

/**
 * Scans all namespace declarations on the document element and returns the
 * prefix bound to the ICGEM namespace URI.
 *
 * It first tries the standard DOM `lookupPrefix()` method (O(1), always
 * correct), then falls back to iterating `documentElement.attributes` for
 * environments where `lookupPrefix` is unavailable.
 *
 * @param {Document} xmlDoc - The parsed XML document
 * @returns {string|null} The prefix (e.g. "grav") or null if not found
 */
function detectIcgemPrefix(xmlDoc) {
  const root = xmlDoc.documentElement;

  // Fast path: standard DOM method
  if (typeof root.lookupPrefix === 'function') {
    const prefix = root.lookupPrefix(ICGEM_NAMESPACE_URI);
    if (prefix) return prefix;
  }

  // Fallback: scan xmlns:* attributes on the root element
 ...
 * @param {string} localName - Local element name (no prefix)
 * @returns {string}
 */
function icgemText(xmlDoc, contextNode, localName) {
  // Try namespaced form first, then fall back to no-namespace
 ...
 * Returns true if an option was matched and selected.
 * @param {jQuery} $select
 * @param {string} text
 * @returns {boolean}
 */
...
/**
 * Populates the GGMsDefinition form fields from an ICGEM XML document.
 * Reads modelName, modelType, mathematicalRepresentation, fileFormat, celestialBody.
 * Triggers 'change' on model-type so downstream visibility handlers fire.
 * @param {Document} xmlDoc
 */
function processIcgemDefinition(xmlDoc) {
  const shm = xmlDoc.evaluate(
    './/icgv:harmonicCoefficientsModel | .//harmonicCoefficientsModel',
    xmlDoc, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (!shm) return;

  const modelName = icgemText(xmlDoc, shm, 'modelName');
  if (modelName) $('#input-model-name').val(modelName);

  const modelType = icgemText(xmlDoc, shm, 'modelType');
  if (modelType) {
    // Try direct val() first (options populated from API), fall back to text match
    const $select = $('#input-model-type');
    if (!selectOptionByText($select, modelType)) {
      $select.val(modelType);
    }
    $select.trigger('change');
  }

  const mathRep = icgemText(xmlDoc, shm, 'mathematicalRepresentation');
  if (mathRep) {
    const $select = $('#input-mathematical-representation');
    if (!selectOptionByText($select, mathRep)) {
      $select.val(mathRep);
    }
    $select.trigger('change');
  }

  const fileFormat = icgemText(xmlDoc, shm, 'fileFormat');
  if (fileFormat) {
    const $select = $('#input-file-format');
    if (!selectOptionByText($select, fileFormat)) {
      $select.val(fileFormat);
    }
  }

  const celestialBody = icgemText(xmlDoc, shm, 'celestialBody');
  if (celestialBody) $('#input-celestial-body').val(celestialBody);
}

/**
 * Populates the GGMsProperties form fields from an ICGEM XML document.
 * Reads tideSystem, degree, errors, errorHandling, radius, earthGravityConstant,
 * and ellipsoidal parameters if present.
 * @param {Document} xmlDoc
 */
function processIcgemProperties(xmlDoc) {
  const shm = xmlDoc.evaluate(
    './/icgv:harmonicCoefficientsModel | .//harmonicCoefficientsModel',
    xmlDoc, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  ...
  // Tide system: XML stores "tide-free", form expects "tide free" etc.
  const tideSystem = icgemText(xmlDoc, shm, 'tideSystem').replace(/-/g, ' ');
  if (tideSystem) $('#input-tide-system').val(tideSystem);

  const degree = icgemText(xmlDoc, shm, 'degreeOrderMax');
  if (degree) $('#input-degree').val(degree);

  // Errors: XML stores "Formal" (ucfirst), select values are lowercase
  const errors = icgemText(xmlDoc, shm, 'errors').toLowerCase();
  if (errors) $('#input-errors').val(errors);

  const errorHandling = icgemText(xmlDoc, shm, 'errorHandling');
  if (errorHandling) $('#input-error-handling-approach').val(errorHandling);

  const radius = icgemText(xmlDoc, shm, 'radius');
  if (radius) $('#input-radius').val(radius);

  const egc = icgemText(xmlDoc, shm, 'earthGravityConstant');
  if (egc) $('#input-earth-gravity-constant').val(egc);

  // Ellipsoidal parameters
  const epNode = xmlDoc.evaluate(
    './/icgv:ellipsoidalParameters | .//ellipsoidalParameters',
    shm, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (epNode) {
    const semimajorA = icgemText(xmlDoc, epNode, 'semimajorAxisA');
    if (semimajorA) $('#input-semimajor-axis').val(semimajorA);

    const semiminorB = icgemText(xmlDoc, epNode, 'semiminorAxisB');
    const flattening = icgemText(xmlDoc, epNode, 'flattening');
    const reciprocalFlattening = icgemText(xmlDoc, epNode, 'reciprocalFlattening');
    const eccentricity = icgemText(xmlDoc, epNode, 'eccentricity');

    if (semiminorB) {
      $('#input-second-variable').val('axis_b');
      $('#input-second-variable-value').val(semiminorB);
    } else if (reciprocalFlattening) {
      $('#input-second-variable').val('reciprocal_flattening');
      $('#input-second-variable-value').val(reciprocalFlattening);
    } else if (flattening) {
      $('#input-second-variable').val('flattening');
      $('#input-second-variable-value').val(flattening);
    } else if (eccentricity) {
      $('#input-second-variable').val('eccentricity');
      $('#input-second-variable-value').val(eccentricity);
    }
  }
}

/**
 * Reverse-maps ICGEM densityInformationType enumeration values back to select option values.
 * The XML stores "Constant", "Layer-specific", "Density model" (after PHP prepare()).
 * The form option values are "constant", "layer-specific", "density-model".
...
/**
 * Populates the GGMsModelTypes form fields from an ICGEM XML document.
 * Handles static, temporal, and topographic model type-specific properties.
 * @param {Document} xmlDoc
 */
function processIcgemModelTypes(xmlDoc) {
  const shm = xmlDoc.evaluate(
    './/icgv:harmonicCoefficientsModel | .//harmonicCoefficientsModel',
    xmlDoc, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  ...
  const smpNode = xmlDoc.evaluate(
    './/icgv:staticModelProperties | .//staticModelProperties',
    shm, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (smpNode) {
    const infoTVC = icgemText(xmlDoc, smpNode, 'infoTimeVariableCoefficients');
    if (infoTVC) {
      $('#checkbox-time-variable').prop('checked', true);
      $('#time-variable-description-container').removeClass('d-none');
      $('#input-static-description').val(infoTVC);
    }
  }

  // Temporal model properties
  const tmpNode = xmlDoc.evaluate(
    './/icgv:temporalModelProperties | .//temporalModelProperties',
    shm, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (tmpNode) {
    const startDate = icgemText(xmlDoc, tmpNode, 'startDate');
    const stopDate = icgemText(xmlDoc, tmpNode, 'stopDate');
    const generatingInstitution = icgemText(xmlDoc, tmpNode, 'generatingInstitution');
    const release = icgemText(xmlDoc, tmpNode, 'release');
    const temporalResolution = icgemText(xmlDoc, tmpNode, 'temporalResolution');

    if (startDate) $('#input-temporal-start').val(startDate);
    if (stopDate) $('#input-temporal-end').val(stopDate);
    if (generatingInstitution) $('#input-temporal-institution').val(generatingInstitution);
    if (release) $('#input-release-number').val(release);

    if (temporalResolution) {
      const days = parseInt(temporalResolution, 10);
      const predefMap = { 1: 'daily', 7: 'weekly', 30: 'monthly', 91: 'quarterly', 365: 'yearly' };
      if (predefMap[days]) {
        $('#select-temporal-frequency-predef').val(predefMap[days]);
      } else if (!isNaN(days)) {
        $('#checkbox-custom-frequency').prop('checked', true);
        $('#custom-frequency-container').removeClass('d-none');
        $('#input-temporal-frequency').val(temporalResolution);
      }
    }
  }

  // Topographic model properties
  const topNodes = xmlDoc.evaluate(
    './/icgv:topographicModelProperties | .//topographicModelProperties',
    shm, icgemResolver, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  if (topNodes.snapshotLength > 0) {
    const topNode = topNodes.snapshotItem(0);

    const layerApproach = icgemText(xmlDoc, topNode, 'layerApproach').toLowerCase();
    const domain = icgemText(xmlDoc, topNode, 'forwardModellingDomain').toLowerCase();
    const approximation = icgemText(xmlDoc, topNode, 'approximation').toLowerCase();

    if (layerApproach) $('#select-topo-layerapproach').val(layerApproach);
    if (domain) $('#select-topo-domain').val(domain);
    if (approximation) $('#select-topo-approximation').val(approximation);

    const densityNodes = xmlDoc.evaluate(
      './/icgv:densityInformation | .//densityInformation',
      topNode, icgemResolver, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
    );
    for (let d = 0; d < densityNodes.snapshotLength; d++) {
      const densityNode = densityNodes.snapshotItem(d);
      const densityDomain = icgemText(xmlDoc, densityNode, 'densityInformationDomain').toLowerCase();
      const infoType = reverseDensityType(icgemText(xmlDoc, densityNode, 'densityInformationType'));
      const description = icgemText(xmlDoc, densityNode, 'densityInformationDescription');

      if (densityDomain === 'whole') {
        $('#select-topo-density').val(infoType);
        if (description) $('#input-topo-density-details').val(description);
      } else if (densityDomain === 'crust') {
...
/**
 * Populates the GGMsDataSources form rows from an ICGEM XML document.
 * Each inputDataSource element in icgv:globalGravityProduct becomes one form row.
 * Triggers the datasource type 'change' event so row visibility updates correctly.
 * @param {Document} xmlDoc
 */
function processIcgemDataSources(xmlDoc) {
  const globalGravityProduct = xmlDoc.evaluate(
    './/icgv:globalGravityProduct | .//globalGravityProduct',
    xmlDoc, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
  ).singleNodeValue;
  if (!globalGravityProduct) return;

  const dsNodes = xmlDoc.evaluate(
    'icgv:inputDataSource | inputDataSource',
    globalGravityProduct, icgemResolver, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );
  if (dsNodes.snapshotLength === 0) return;
  ...
  for (let i = 0; i < dsNodes.snapshotLength; i++) {
    const dsNode = dsNodes.snapshotItem(i);

    if (i > 0) {
      // Add a new datasource row by clicking the last add button
      $('.addDataSource').last().trigger('click');
    }

    const $row = $('[data-source-row]').last();

    const typeText = icgemText(xmlDoc, dsNode, 'inputDataSourceType').toLowerCase();
    const typeCode = typeNameToCode[typeText] || 'S';
    const description = icgemText(xmlDoc, dsNode, 'description');

    $row.find('select[name="datasource_type[]"]').val(typeCode).trigger('change');

    if (description) $row.find('textarea[name="datasource_description[]"]').val(description);

    if (typeCode === 'S') {
      const satName = icgemText(xmlDoc, dsNode, 'satelliteValueName');
      const satUri = icgemText(xmlDoc, dsNode, 'satelliteValueUri');
      const schemeName = icgemText(xmlDoc, dsNode, 'satelliteSchemeName');
      const schemeUri = icgemText(xmlDoc, dsNode, 'satelliteSchemeUri');
      if (satName) {
        const platformInput = $row.find('input[name="satellite_platform[]"]')[0];
        if (platformInput && platformInput._tagify) {
          platformInput._tagify.addTags([{ value: satName, id: satUri, scheme: schemeName, schemeURI: schemeUri }]);
        } else if (platformInput) {
          // Fallback: store JSON payload directly (mirrors save format)
          $(platformInput).val(JSON.stringify([{ value: satName, id: satUri, scheme: schemeName, schemeURI: schemeUri }]));
        }
      }
    } else if (typeCode === 'G') {
      const detail = icgemText(xmlDoc, dsNode, 'groundDetail');
      if (detail) $row.find('select[name="datasource_details[]"]').val(detail);
    } else if (typeCode === 'A') {
      const detail = icgemText(xmlDoc, dsNode, 'altimetryDetail');
      if (detail) $row.find('select[name="datasource_details[]"]').val(detail);
    } else if (typeCode === 'T') {
      const detail = icgemText(xmlDoc, dsNode, 'elevationTerrainDetail');
      if (detail) $row.find('select[name="datasource_details[]"]').val(detail).trigger('change');
      const compNode = xmlDoc.evaluate(
        './/icgv:compensationDepth | .//compensationDepth',
        dsNode, icgemResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null
      ).singleNodeValue;
      if (compNode) {
        $row.find('input[name="compensation_depth[]"]').val(compNode.textContent.trim());
      }
    } else if (typeCode === 'M') {
      const detail = icgemText(xmlDoc, dsNode, 'modelDetail');
      if (detail) $row.find('select[name="datasource_details[]"]').val(detail);
      const identifier = icgemText(xmlDoc, dsNode, 'identifier');
      const identifierType = icgemText(xmlDoc, dsNode, 'identifierType');
      const name = icgemText(xmlDoc, dsNode, 'name');
      if (identifier) $row.find('input[name="dIdentifier[]"]').val(identifier);
      if (identifierType) {
        const $idTypeSelect = $row.find('select[name="dIdentifierType[]"]');
        if (!selectOptionByText($idTypeSelect, identifierType)) {
          $idTypeSelect.val(identifierType);
        }
      }
      if (name) $row.find('input[name="dName[]"]').val(name);
    }
  }
}

/**
 * Populates the GGMsDescriptions form fields from an ICGEM XML document.
 * The ICGEM descriptions element uses a 'section' attribute instead of the
 * DataCite 'descriptionType'. Sections map directly to dedicated textarea IDs.
 * @param {Document} xmlDoc
 */
function processIcgemDescriptions(xmlDoc) {
  // Descriptions may be namespaced (http://icgem.gfz.de/schema) or in no namespace
  const descNodes = xmlDoc.evaluate(
    './/icgv:globalGravityProduct//icgv:description[@section] | .//icgv:globalGravityProduct//description[@section]',
    xmlDoc, icgemResolver, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null
  );

  // Map of section attribute value (lowercase) → textarea element ID
  const sectionToId = {
    'abstract': 'input-abstract',
  ...
  // Map of section → accordion collapse ID to expand
  ...
  for (let i = 0; i < descNodes.snapshotLength; i++) {
    const descNode = descNodes.snapshotItem(i);
    const section = (descNode.getAttribute('section') || '').toLowerCase();
    const content = descNode.textContent.trim();
...
async function loadXmlToForm(xmlDoc) {
  ...
  const isIcgem = detectXmlSchema(xmlDoc) === 'icgem';
  if (isIcgem) {
    processIcgemDescriptions(xmlDoc);
  } else {
  ...
  // For ICGEM schema files, populate GGM-specific formgroups
  if (isIcgem) {
    processIcgemDefinition(xmlDoc);
    processIcgemProperties(xmlDoc);
    processIcgemModelTypes(xmlDoc);
    processIcgemDataSources(xmlDoc);
  }
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        ...
        selectOptionByText,
        detectXmlSchema,
        processIcgemDefinition,
        processIcgemProperties,
        processIcgemModelTypes,
        processIcgemDataSources,
        processIcgemDescriptions
    };
}
