...
final class GGMsRegistrationMailTest extends TestCase
{
    ...
    public function testDataServicesNoteNamesIcgemAddress(): void
    {
        ...
        $this->assertStringContainsString('ELMO-GEM', $note['html']);
        $this->assertStringContainsString('icgem@gfz.de', $note['html']);
        $this->assertStringContainsString('icgem@gfz.de', $note['text']);
    }

    public function testIcgemMessageWaitsForDoiWhenDataServicesWasNotified(): void
    {
        ...
        $this->assertStringContainsString('ELMO-GEM', $message['subject']);
        $this->assertStringContainsString('ELMO-GEM', $message['html']);
        $this->assertStringContainsString('GFZ Data Services email sent:</strong> true', $message['html']);
        $this->assertStringContainsString('wait for the DOI generated by GFZ Data', $message['html']);
        $this->assertStringContainsString('wait for the DOI generated by GFZ Data Services', $message['text']);
        $this->assertStringContainsString(GGMS_ICGEM_DATABASE_URL, $message['html']);
        $this->assertStringContainsString(GGMS_ICGEM_DATABASE_URL, $message['text']);
        $this->assertStringNotContainsString('can start straight away', $message['html']);
        $this->assertSame('metadata42.xml', $message['attachments'][0]['filename']);
    }

    public function testIcgemMessageAllowsImmediateUploadWhenDoiExists(): void
    {
        $message = buildGGMsIcgemMessage(
        ...
        $this->assertStringContainsString('can start straight away', $message['html']);
        $this->assertStringContainsString(GGMS_ICGEM_DATABASE_URL, $message['html']);
        $this->assertStringNotContainsString('wait for the DOI', $message['html']);
    }
    ...
    public function testPlainTextBodyMirrorsDoiStatement(): void
    {
        ...
        $this->assertStringContainsString('wait for the DOI generated by GFZ Data Services', $sentMessage['text']);
        ...
        $this->assertStringContainsString('can start straight away', $notSentMessage['text']);
        $this->assertStringContainsString(GGMS_ICGEM_DATABASE_URL, $notSentMessage['text']);
    }
