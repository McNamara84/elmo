...
describe("select.js", () => {
  ...
  test("setupIdentifierTypesDropdown populates options", () => {
    $.getJSON.mockImplementationOnce((url, cb) => {
      cb({
        identifierTypes: [
          { name: "DOI", description: "d1" },
          { name: "HANDLE", description: "d2" },
        ],
      });
      return { fail: jest.fn() };
    });
    window.setupIdentifierTypesDropdown("#test-select");
    const options = $("#test-select option")
      .map((i, el) => $(el).text())
      .get();
  ...
  test("setupIdentifierTypesDropdown warns when no types", () => {
    ...
    $.getJSON.mockImplementationOnce((u, cb) => {
      cb({});
      return { fail: jest.fn() };
    });
    window.setupIdentifierTypesDropdown("#test-select");
  ...
  test("updateIdentifierType handles invalid regex", async () => {
    const warnSpy = jest.spyOn(console, "warn").mockImplementation(() => {});
    $.ajax.mockImplementationOnce((opts) => {
      opts.success({ identifierTypes: [{ name: "BAD", pattern: "(/" }] });
      return { fail: jest.fn() };
    });
    const input = $("#group-relatedwork .row:first-child input");
    input.val("bad");
  ...
  test("updateIdsAndNames assigns sequential ids", () => {
    ...
    const ids = $('#group-relatedwork select[name^="relation"]')
      .map((i, el) => $(el).attr("id"))
      .get();
