...
describe('mappingXmlToInputFields module coverage', () => {
    ...
    describe('processKeywords', () => {
        ...
        test('resolves a short XML subject to the full whitelist path via valueURI and merges it into the whitelist', () => {
            document.body.innerHTML = `
            <input id="input-platforms">
        `;

            const platformsTagify = {
                removeAllTags: jest.fn(),
                addTags: jest.fn(),
                settings: {
                    whitelist: [{
                        value: 'Platforms > Space-based Platforms > Earth Observation Satellites > GRACE',
            ...
            document.querySelector('#input-platforms')._tagify = platformsTagify;

            const xmlDoc = new DOMParser().parseFromString(`
            <ns:resource xmlns:ns="http://datacite.org/schema/kernel-4">
                <ns:subjects>
                    <ns:subject
                        subjectScheme="NASA/GCMD Earth Platforms Keywords"
                        schemeURI="https://gcmd.earthdata.nasa.gov/kms/concepts/concept_scheme/platforms"
                        valueURI="https://gcmd.earthdata.nasa.gov/kms/concept/2e7aa2e6-9d25-4c6e-aef3-6e86d3773bac"
                        xml:lang="en">Space-based Platforms &gt; Earth Observation Satellites &gt; GRACE</ns:subject>
                </ns:subjects>
            </ns:resource>
        `, 'text/xml');
            ...
            expect(platformsTagify.addTags).toHaveBeenCalledWith([
                expect.objectContaining({
                    value: 'Platforms > Space-based Platforms > Earth Observation Satellites > GRACE',
                    id: 'https://gcmd.earthdata.nasa.gov/kms/concept/2e7aa2e6-9d25-4c6e-aef3-6e86d3773bac',
                })
            ]);
            expect(platformsTagify.settings.whitelist.some(
                (entry) => entry.value === 'Platforms > Space-based Platforms > Earth Observation Satellites > GRACE'
            )).toBe(true);
        });

        test('resolves a short XML subject by suffix when valueURI is missing', () => {
            document.body.innerHTML = `
            <input id="input-sciencekeyword">
        `;
            ...
            const scienceTagify = {
                ...
                settings: {
                    whitelist: [{
                        value: 'Science Keywords > EARTH SCIENCE > SOLID EARTH > GEODETICS',
                        id: 'https://gcmd.earthdata.nasa.gov/kms/concept/5498572c-aaed-4c08-8aad-8b297057e9c9',
                    }]
                }
            };
            ...
            <ns:resource xmlns:ns="http://datacite.org/schema/kernel-4">
                <ns:subjects>
                    <ns:subject schemeURI="https://gcmd.earthdata.nasa.gov/kms/concepts/concept_scheme/sciencekeywords">GEODETICS</ns:subject>
                </ns:subjects>
            </ns:resource>
        `, 'text/xml');
