...
function validateContactPerson() {
  ...
  if (!isValid) {
    $("#group-author").append(
      '<div id="contact-person-error" class="text-danger mt-2" data-translate="contactPersons.contactPersonError"></div>'
    );
    applyTranslations();
...
class SaveHandler {
  ...
  initializeEventListeners() {
    ...
    $("#modal-saveas").on("keydown", (e) => {
      ...
      if (e.which === 13 || e.keyCode === 13) {
        ...
        if (activeElement.classList.contains("btn-secondary") || activeElement.classList.contains("btn-close")) {
          return;
        }
  ...
  async handleSave() {
    ...
    if (!this.$form[0].checkValidity() || !validateContactPerson()) {
      ...
      $firstInvalid.focus();
      this.showNotification("danger", translations.alerts.validationErrorheading, translations.alerts.validationError);
      return;
    }

    this.showNotification("info", translations.alerts.processingHeading, translations.alerts.preparingDownload);
  ...
  async generateFilename() {
    try {
      const timestamp = new Date().toISOString().replace(/[-:]/g, "").replace(/[T.]/g, "_").slice(0, 15);
      return `dataset_${timestamp}`;
    } catch (error) {
      console.error("Error generating filename:", error);
      this.showNotification("danger", translations.alerts.errorHeading, translations.alerts.filenameGenerationError);
      return null;
    }
  }
  ...
  async handleSaveConfirm() {
    ...
    if (!filename) {
      this.showNotification("danger", translations.alerts.filenameErrorHeading, translations.alerts.filenameError);
      return;
    }
  ...
  async saveAndDownload(filename) {
    ...
    this.showNotification("info", translations.alerts.savingHeading, translations.alerts.savingInfo);
    ...
    try {
      ...
      this.showNotification("success", translations.alerts.successHeading, translations.alerts.savingSuccess);
    } catch (error) {
      console.error("Error saving dataset:", error);
      this.showNotification("danger", translations.alerts.errorHeading, translations.alerts.saveError);
    }
  }
